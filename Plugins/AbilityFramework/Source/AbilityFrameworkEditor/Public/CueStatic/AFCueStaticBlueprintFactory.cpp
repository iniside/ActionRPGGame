...
#include "AFCueStaticBlueprintFactory.h"
...
#include "Cues/AFCueStaticGeneratedClass.h"
...
public:
	...
	bool ConfigureProperties(TWeakObjectPtr<UAFCueStaticBlueprintFactory> InEffectCueBlueprintFactory)
	{
...
private:
	...
	TWeakObjectPtr<UAFCueStaticBlueprintFactory> EffectCueBlueprintFactory;
...
UAFCueStaticBlueprintFactory::UAFCueStaticBlueprintFactory(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	bCreateNew = true;
	bEditAfterNew = true;
	SupportedClass = UAFCueStaticBlueprint::StaticClass();
	ParentClass = UAFCueStatic::StaticClass();
}

bool UAFCueStaticBlueprintFactory::ConfigureProperties()
{
	TSharedRef<SEffectCueBlueprintCreateDialog> Dialog = SNew(SEffectCueBlueprintCreateDialog);
	return Dialog->ConfigureProperties(this);
};

UObject* UAFCueStaticBlueprintFactory::FactoryCreateNew(UClass* Class, UObject* InParent, FName Name, EObjectFlags Flags, UObject* Context, FFeedbackContext* Warn, FName CallingContext)
{
	// Make sure we are trying to factory a gameplay ability blueprint, then create and init one
	check(Class->IsChildOf(UAFCueStaticBlueprint::StaticClass()));
	...
	{
		UAFCueStaticBlueprint* NewBP = CastChecked<UAFCueStaticBlueprint>(FKismetEditorUtilities::CreateBlueprint(ParentClass, InParent, Name, BlueprintType, UAFCueStaticBlueprint::StaticClass(), UAFCueStaticGeneratedClass::StaticClass(), CallingContext));

		if (NewBP)
		{
//			UAFCueStaticBlueprint* AbilityBP = UAFCueActorBlueprint::FindRootGameplayAbilityBlueprint(NewBP);
//			if (AbilityBP == NULL)
//			{
...
UObject* UAFCueStaticBlueprintFactory::FactoryCreateNew(UClass* Class, UObject* InParent, FName Name, EObjectFlags Flags, UObject* Context, FFeedbackContext* Warn)
{
