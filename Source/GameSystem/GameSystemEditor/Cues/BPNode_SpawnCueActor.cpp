...
#include "BPNode_SpawnCueActor.h"
...
struct FBPNode_SpawnCueActorHelper
{
...
FString FBPNode_SpawnCueActorHelper::EffectClassPinName(TEXT("CueActorClass"));
FString FBPNode_SpawnCueActorHelper::LocationPinName(TEXT("Location"));
FString FBPNode_SpawnCueActorHelper::InstigatorPinName(TEXT("CueInstigator"));
...
void UBPNode_SpawnCueActor::AllocateDefaultPins()
{
	...
	UEdGraphPin* EffectClassPin = CreatePin(EGPD_Input, K2Schema->PC_Class, TEXT(""), AGSPersistentCue::StaticClass(), false, false, FBPNode_SpawnCueActorHelper::EffectClassPinName);
	...
	UEdGraphPin* LocationPin = CreatePin(EGPD_Input, K2Schema->PC_Struct, TEXT(""), VectorStruct, false, false, FBPNode_SpawnCueActorHelper::LocationPinName);
...
bool UBPNode_SpawnCueActor::IsSpawnVarPin(UEdGraphPin* Pin)
{
	const UEdGraphSchema_K2* K2Schema = GetDefault<UEdGraphSchema_K2>();
	...
	return(Pin->PinName != K2Schema->PN_Execute &&
		...
		Pin->PinName != FBPNode_SpawnCueActorHelper::EffectClassPinName &&
		Pin->PinName != FBPNode_SpawnCueActorHelper::LocationPinName &&
		Pin->PinName != FBPNode_SpawnCueActorHelper::InstigatorPinName);
}

void UBPNode_SpawnCueActor::PinDefaultValueChanged(UEdGraphPin* Pin)
{
	if (Pin && (Pin->PinName == FBPNode_SpawnCueActorHelper::EffectClassPinName))
	{
		OnClassPinChanged();
	}
}
void UBPNode_SpawnCueActor::PinConnectionListChanged(UEdGraphPin* Pin)
{
	if (Pin && (Pin->PinName == FBPNode_SpawnCueActorHelper::EffectClassPinName))
	{
		OnClassPinChanged();
...
UClass* UBPNode_SpawnCueActor::GetClassToSpawn(const TArray<UEdGraphPin*>* InPinsToSearch /*=NULL*/) const
{
...
void UBPNode_SpawnCueActor::OnClassPinChanged()
{
	const UEdGraphSchema_K2* K2Schema = GetDefault<UEdGraphSchema_K2>();
...
FLinearColor UBPNode_SpawnCueActor::GetNodeTitleColor() const
{
...
UEdGraphPin* UBPNode_SpawnCueActor::GetThenPin() const
{
	const UEdGraphSchema_K2* K2Schema = GetDefault<UEdGraphSchema_K2>();
...
UEdGraphPin* UBPNode_SpawnCueActor::GetClassPin(const TArray<UEdGraphPin*>* InPinsToSearch) const
{
	const TArray<UEdGraphPin*>* PinsToSearch = InPinsToSearch ? InPinsToSearch : &Pins;
	...
	for (auto PinIt = PinsToSearch->CreateConstIterator(); PinIt; ++PinIt)
	{
		...
		if (TestPin && TestPin->PinName == FBPNode_SpawnCueActorHelper::EffectClassPinName)
		{
			Pin = TestPin;
			break;
		}
...
UEdGraphPin* UBPNode_SpawnCueActor::GetLocationPin() const
{
	UEdGraphPin* Pin = FindPin(FBPNode_SpawnCueActorHelper::LocationPinName);
	check(Pin == NULL || Pin->Direction == EGPD_Input);
	return Pin;
}

UEdGraphPin* UBPNode_SpawnCueActor::GetInstigatorPin() const
{
	UEdGraphPin* Pin = FindPin(FBPNode_SpawnCueActorHelper::InstigatorPinName);
	check(Pin == NULL || Pin->Direction == EGPD_Input);
	return Pin;
}

UEdGraphPin* UBPNode_SpawnCueActor::GetResultPin() const
{
	const UEdGraphSchema_K2* K2Schema = GetDefault<UEdGraphSchema_K2>();
...
void UBPNode_SpawnCueActor::GetNodeAttributes(TArray<TKeyValuePair<FString, FString>>& OutNodeAttributes) const
{
...
void UBPNode_SpawnCueActor::ExpandNode(class FKismetCompilerContext& CompilerContext, UEdGraph* SourceGraph)
{
	UK2Node::ExpandNode(CompilerContext, SourceGraph);

	//look for static function in BlueprintFunctionLibrary
	//In this class and of this name
	static FName BeginSpawn_FunctionName = GET_FUNCTION_NAME_CHECKED(UGSBlueprintFunctionLibrary, BeginSpawnCueActor);


	static FName FinishSpawn_FunctionName = GET_FUNCTION_NAME_CHECKED(UGSBlueprintFunctionLibrary, FinishSpawnCueActor);
	...
	static const FString EffectClassType_Param = FBPNode_SpawnCueActorHelper::EffectClassPinName;
	static const FString LocationType_Param = FBPNode_SpawnCueActorHelper::LocationPinName;
	static const FString InstigatorType_Param = FBPNode_SpawnCueActorHelper::InstigatorPinName;
	//get pointer to self;
	UBPNode_SpawnCueActor* CreateEffectNode = this;
